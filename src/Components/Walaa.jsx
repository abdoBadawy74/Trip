...
function Walaa() {
    ...
    const getData = () => {
        axios.get(`${BASE}/hotels`)
            .then((res) => {
                setProducts(res.data.data);
                const initialFiltered = res.data.data.filter(product => product.state.name === state);
    ...
    const handleSearch = () => {
        if (searchState === '') {
            ...
            const filtered = products.filter(product => product.state.name.toLowerCase().includes(searchState.toLowerCase()));
            setFilteredProducts(filtered);
        }
    };
            ...
            <div className="container">
                <div className="row">
                    <div className="d-flex justify-content-between align-items-start mb-4 mt-4 pt-4">
                        <div>
                            <h2 className="text-uppercase" style={{ fontWeight: 600, letterSpacing: "2px", fontSize: "24px", color:" #42A7C3" }}>
                                All Hotels
                            </h2>
                        </div>
                        <div>
                            <button
                                className={`btn ${state === "Abu Dhabi" ? "bg-dark text-white" : "btn-light"} mx-2`}
                                onClick={() => setState("Abu Dhabi")}
                            >
                                Abu Dhabi
                            </button>
                            <button
                                className={`btn ${state === "Dubai" ? "bg-dark text-white" : "btn-light"} mx-2`}
                    ...
                    <div className="search container mt-5 d-flex ">
                        <input
                            ...
                            style={{ backgroundColor:" #F8F9F9", border:"none", outline:"none" }}
                            value={searchState} 
                            onChange={(e) => setSearchState(e.target.value)}
                        />
                        <button className="btn m-2" style={{backgroundColor: "#42A7C3" , color:"#ffff"}} onClick={handleSearch}>
                            Search
                        </button>
                    </div>
                            ...
                            <div className="box bg-white p-2 rounded">
                                <div className="image p-2">
                                    {product.images.length > 0 && (
                                        <img src={product.images[0].url} alt={product.name} className="w-100 rounded" />
                                    )}
                                </div>
                                <div className="text px-2">
                                    <div className="d-flex align-items-center">
                                        <h3 className="fw-bold">{product.name}</h3>
                                        <p className="ms-auto fs-2 fw-bold" style={{ color: "#F77A40" }}>
                                            45HD
                                        </p>
                                    </div>
                                    ...
                                    <div className="rating">
                                        <div>
                                            {[...Array(5)].map((star, i) => (
                                                <i key={i} className={`fa-solid fa-star ${i < product.rating ? 'text-warning' : 'text-secondary'} mx-1`}></i>
                                            ))}
                                        </div>
                                    </div>
                                </div>
